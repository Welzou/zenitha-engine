---
description: (Opus) Traite la prochaine tâche non cochée de todo.md, réservé aux tâches taguées [opus] avec le cycle explore → plan → implémente → vérifie → review → commit. À lancer manuellement, une tâche par invocation.
...
- Si la tâche ne porte **pas** le tag `[opus]`, dis-le moi : je devrais utiliser `/next` (Sonnet) pour économiser. Continue quand même si je confirme.
...
- Si la tâche touche `main.py`, la validation d'entrées, ou l'auth par clé : lance `/security-review`. Si la tâche est E08 ou E14, lance aussi l'agent `calc-verifier` et colle son verdict.
